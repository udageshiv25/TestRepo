...
Given(/^I am on the Selenium home page$/, async () => {
  log.debug("Starting");
...
Given(/^I should see the title$/, async () => {
  log.debug("Title is getting displayed");
  await expect(await homePage.getTitleText()).to.equal('Selenium: Beginners Guide');
});
